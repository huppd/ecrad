...
contains
  ...
  subroutine allocate_flux_type(this, config, istartcol, iendcol, nlev)
    ...
    class(flux_type_acc), intent(inout) :: this
    type(config_type), intent(in)   :: config

    real(jprb)                      :: hook_handle

    if (lhook) call dr_hook('radiation_flux_acc:allocate',0,hook_handle)
    ...
    if (config%do_lw) then
      ...
      if (config%do_save_spectral_flux) then
        ...
        allocate(this%lw_up_band(istartcol:iendcol,config%n_spec_lw,nlev+1))
        allocate(this%lw_dn_band(istartcol:iendcol,config%n_spec_lw,nlev+1))
        if (config%do_clear) then
          allocate(this%lw_up_clear_band(istartcol:iendcol, &
               &                         config%n_spec_lw,nlev+1))
          allocate(this%lw_dn_clear_band(istartcol:iendcol, &
               &                         config%n_spec_lw,nlev+1))
        end if
      end if
      ...
      allocate(this%lw_dn_surf_g(istartcol:iendcol,config%n_g_lw))
      if (config%do_clear) then
        allocate(this%lw_dn_surf_clear_g(istartcol:iendcol,config%n_g_lw))
      end if
      ...
      if (config%do_canopy_fluxes_lw) then
        ...
        allocate(this%lw_dn_surf_canopy(istartcol:iendcol,config%n_canopy_bands_lw))
      end if
    end if
    ...
    if (config%do_sw) then
      ...
      if (config%do_save_spectral_flux) then
        ...
        allocate(this%sw_up_band(istartcol:iendcol,config%n_spec_sw,nlev+1))
        allocate(this%sw_dn_band(istartcol:iendcol,config%n_spec_sw,nlev+1))
        
        if (config%do_sw_direct) then
          allocate(this%sw_dn_direct_band(istartcol:iendcol, &
               &                          config%n_spec_sw,nlev+1))
        end if
        if (config%do_clear) then
          allocate(this%sw_up_clear_band(istartcol:iendcol, &
               &                         config%n_spec_sw,nlev+1))
          allocate(this%sw_dn_clear_band(istartcol:iendcol, &
               &                         config%n_spec_sw,nlev+1))
          if (config%do_sw_direct) then
            allocate(this%sw_dn_direct_clear_band(istartcol:iendcol, &
                 &                                config%n_spec_sw, nlev+1))
          end if
        end if
      end if
      ...
      if (config%do_surface_sw_spectral_flux) then
        ...
        allocate(this%sw_dn_surf_band(istartcol:iendcol,config%n_bands_sw))
        allocate(this%sw_dn_direct_surf_band(istartcol:iendcol,config%n_bands_sw))
        if (config%do_clear) then
          allocate(this%sw_dn_surf_clear_band(istartcol:iendcol, &
               &                              config%n_bands_sw))
          allocate(this%sw_dn_direct_surf_clear_band(istartcol:iendcol, &
               &                                     config%n_bands_sw))
        end if
      end if
      ...
      ! Allocate g-point downwelling fluxes at surface passed from
      ! solver to surface_intermediate%partition
      allocate(this%sw_dn_diffuse_surf_g(istartcol:iendcol,config%n_g_sw))
      allocate(this%sw_dn_direct_surf_g (istartcol:iendcol,config%n_g_sw))
      if (config%do_clear) then
        allocate(this%sw_dn_diffuse_surf_clear_g(istartcol:iendcol,config%n_g_sw))
        allocate(this%sw_dn_direct_surf_clear_g (istartcol:iendcol,config%n_g_sw))
      end if

      if (config%do_canopy_fluxes_sw) then
        ! Downward fluxes at top of canopy at the spectral resolution
        ! used in the canopy radiative transfer scheme
        allocate(this%sw_dn_diffuse_surf_canopy(istartcol:iendcol,config%n_canopy_bands_sw))
        allocate(this%sw_dn_direct_surf_canopy (istartcol:iendcol,config%n_canopy_bands_sw))
      end if
    end if
    ...
    if (lhook) call dr_hook('radiation_flux_acc:allocate',1,hook_handle)
  ...
  subroutine deallocate_flux_type(this)
    ...
    if (lhook) call dr_hook('radiation_flux_acc:deallocate',0,hook_handle)
    ...
    if (lhook) call dr_hook('radiation_flux_acc:deallocate',1,hook_handle)
  ...
  subroutine calc_surface_spectral(this, config, istartcol, iendcol)
    ...
    if (lhook) call dr_hook('radiation_flux_acc:calc_surface_spectral',0,hook_handle)
    ...
    if (lhook) call dr_hook('radiation_flux_acc:calc_surface_spectral',1,hook_handle)
  ...
  function out_of_physical_bounds(this, istartcol, iendcol) result(is_bad)
    ...
    if (lhook) call dr_hook('radiation_flux_acc:out_of_physical_bounds',0,hook_handle)
    ...
    if (lhook) call dr_hook('radiation_flux_acc:out_of_physical_bounds',1,hook_handle)
  ...
  subroutine compare_flux_type(this, filename, routinename, lineno)
    ...
    class(flux_type_acc), intent(in)    :: this
